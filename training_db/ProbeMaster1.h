...
class DataSpace
{
	public:
		...
		std::vector<matrix<rgb_pixel>> faceDet(std::vector<CameraData*> camlist,int camid);
		std::tuple<std::vector<UINT8>,std::vector<std::string>> faceRec(std::vector<matrix<rgb_pixel>> faces,std::vector<CameraData*> camlist,int camid);
};
