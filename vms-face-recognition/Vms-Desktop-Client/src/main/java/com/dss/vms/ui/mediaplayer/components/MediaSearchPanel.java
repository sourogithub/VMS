 ...
 *
 */
public class MediaSearchPanel extends JPanel implements Icons {
	...
	private SearchRecordAction<RecordSession> searchAction;

	public MediaSearchPanel(SearchRecordAction<RecordSession> searchAction) {
		...
		cameraList.setRenderer(new ComboBoxCameraRenderer());
			...
			if (selectedItem instanceof VideoCamera) {
				...
				if ((endTime - startTime) >= 0) {
					...
					if (response.isSuccess() && response.getResponse() != null) {
						...
						searchAction.recordsFound(recordSessions);
					}
				} else {
