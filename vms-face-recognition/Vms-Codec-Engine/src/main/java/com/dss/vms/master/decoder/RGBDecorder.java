...
public class RGBDecorder implements NativeConstants, NativeRetun, AbstractDecoder {
	...
	ByteBuffer mappingBuffer;
	ByteBuffer encodedBuffer;
	IntBuffer decodedBuffer;
...
//		LOGGER.info("Decoded buffer cap - " + decodedBuffer.capacity());
//		LOGGER.info("encoded buffer cap- " + encodedBuffer.capacity());
...
//		LOGGER.debug("encoded Frame size - " + encodedFrameSize);
		...
		if ((allocatedEncodedBufferSize - SIZE_OF_BUFFER_SIZE) < encodedFrameSize) {
			...
			//LOGGER.debug("allocation Frame size - " + allocatedEncodedBufferSize);
		}
...
//		LOGGER.debug("Decoded buffer capacity -" + decodedBuffer.capacity());
...
//			LOGGER.info("HEIGHT -> " + height);
			...
			LOGGER.error("INSUFFICIENT MEMORY [allocation required - " + allocatedDecodedBufferSize + " ]");
		...
		LOGGER.debug("DECODE RESPONSE -> " + decodeResponse);
		return false;
	}
