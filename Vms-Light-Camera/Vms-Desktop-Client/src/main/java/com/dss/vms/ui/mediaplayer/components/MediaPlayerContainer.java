...
import com.dss.vms.ui.mediaplayer.actions.RecordRenderer;
...
public class MediaPlayerContainer extends JPanel {
	...
	private class ControlPanel extends JPanel {
		...
		public ControlPanel() {
			play = new JButton(VmsIcons.media_play_icon);
			play.addActionListener(e -> {
				if (currentMediaIndex != INVALID_MEDIA) {
					String mediaUrl = model.get(currentMediaIndex).getMediaUrl();
					boolean success = playMedia(mediaUrl);

					if (success) timerAction.startAction();
				}
			});
			
			JButton pause = new JButton(VmsIcons.media_pause_icon);
			pause.addActionListener(e -> { 
				pauseMedia(); 
			});
			
			JButton replay = new JButton(VmsIcons.media_replay_icon);
...
//			});
			...
			JButton previous = new JButton(VmsIcons.media_prev_icon);
			previous.addActionListener(e -> {
				boolean success = playPrevious();
				if(success) timerAction.startAction();
			});
			
			JButton next = new JButton(VmsIcons.media_next_icon);
			next.addActionListener(e -> {
				boolean sucess = playNext();
				if(sucess) timerAction.startAction();
			});
			...
			JButton screenshot = new JButton(VmsIcons.media_screenshot_icon);
			screenshot.addActionListener(e->{ saveScreenshot(); });
			
			JButton download = new JButton(VmsIcons.media_download_icon);
			...
			setBackground(VmsComponentColor.MEDIA_CONTROLS_BG);
		...
		private void downloadFile() {
			if(currentMediaIndex == INVALID_MEDIA) showErrorDialog("Error", "No media playing at this moment.");
				...
				if (mediaFile.exists()) {
					...
					fileChooser.setForeground(VmsComponentColor.LABEL_COLOR);
							...
							SwingUtilities.invokeLater(new Runnable() {
								...
								public void run() {
									...
									try {
										...
										if(upload != null) try { upload.close(); } catch (Exception e) {}
									}
								}
							});
						} catch (Throwable e) {
