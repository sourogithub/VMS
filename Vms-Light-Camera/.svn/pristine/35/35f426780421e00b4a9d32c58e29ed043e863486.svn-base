...
import com.dss.vms.common.data.AnalyticSession;
...
public class SessionManagerImpl extends Observable implements Observer, SessionManager {
	...
	// Switches
	private static final boolean PROBE_ENABLE = false;
	private static final boolean STORAGE_MANAGER_ENABLE = true;
	private static final boolean SESSION_STORE_ENABLE = true;
	private static final boolean WEB_SERVER_ENABLE = true;
	private static final boolean REMOTE_STREAMING_SERVER_ENABLE = true;
	
	/* CMap<CamID, VideoCamera> */
	private Map<Integer, VideoCamera> cameraMap = new ConcurrentHashMap<Integer, VideoCamera>();
	
	private Map<Integer, AbstractOperation> pendingOperations = new ConcurrentHashMap<Integer, AbstractOperation>();
	...
	public VmsResponse addCamera(VideoCamera... cameras) {
		...
		if (SESSION_STORE_ENABLE) {
			serializateToXML(cameraID, "id_generator.xml");
	...
	public VmsResponse deleteCamera(Integer... cameraIDs) {
		...
		/** stopping any pending operation on camera **/
		for(Integer cameraId : cameraIDs) {
			try {
				AbstractOperation operation = pendingOperations.get(cameraId);
				if (operation != null) { operation.stop(); }
				pendingOperations.remove(cameraId);
	...
	public VmsResponse sleep() {
		...
		if (PROBE_ENABLE) {
			EventProcessor.getInstance().requestShutdown();
		}
	 ...
	 * @return
	 */
	private boolean serializateToXML(Object object, String fileName) {
	...
	public VmsResponse wakeup() {
		...
		try {
			if (SESSION_STORE_ENABLE) {
				Object serialCameraID = deserializeFromXML("id_generator.xml");
				if(serialCameraID != null) {
					cameraID = (AtomicInteger) serialCameraID; 
				}
				
				Object serialMapObject = deserializeFromXML("camera_session.xml");
				if (serialMapObject != null) {
					cameraMap = (Map<Integer, VideoCamera>) serialMapObject;
				}
				...
				if (cameraMap != null) {
					for (VideoCamera camera : cameraMap.values()) {
						/** adding cameras to media server **/
						VmsResponse addResponse = mediaServer.addCamera(camera);
						
						/** if camera add is failed , start a retry Thread **/
						if(!addResponse.isSuccess()) {
							CameraAddOperation operation = new CameraAddOperation(mediaServer, camera);
							LOGGER.info("Adding operation to pending operations");
							pendingOperations.put(camera.getId(), operation);
							
							Thread thread = new Thread(operation);
							thread.setName("camera add retry Thread [ " + camera.getIp() + "]");
							thread.start();
						}
						
						/** adding cameras to image probe **/
						if (PROBE_ENABLE) {
							VmsResponse probeResponse = imageProbe.addCamera(camera.getId());
							if (!probeResponse.isSuccess()) {
								String logMessage = "\nFailed to add cameras to image probe... "
										+ "No analytics will be available for the camera "
										+ "[\n" + camera + "\n] Error Details: " + probeResponse;
								LOGGER.error(logMessage);
							}
						}
	...
	@Override
	public VmsResponse createAudit(AuditLog type, String desc, Long time) {
	...
	@Override
	public VmsResponse getEventLogBetween(Long startTime, Long endTime) {
		return dbManager.getEventLogBetween(startTime, endTime);
	}

	@Override
	public VmsResponse getEventLogByType(AnalyticType type, Integer limit) {
		return new VmsResponse(CommonResponseCode.NOT_IMPLEMENTED);
	}
