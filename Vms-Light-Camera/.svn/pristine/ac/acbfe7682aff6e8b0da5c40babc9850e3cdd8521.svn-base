...
/**
 * this class is responsible for drawing ImageTiles on registered View Panel
 * @author dss-02 
 * 
 */
public class VmsGridPainter {

	public static final VmsGridLayout DEFAULT_GRID_LAYOUT = VmsGridLayout.Layout2x2;
	
	private ImageTileBucket tileBucket = ImageTileBucket.getInstance();
	private LiveViewPanel view = null;
	private VmsGridLayout currentGridLayout = DEFAULT_GRID_LAYOUT;
	private ImageTile currentSelectedTile = null;

	private static VmsGridPainter INSTANCE = null;
	/**
	 * @return ViewController instance
	 */
	public static VmsGridPainter getInstance() {
		if(INSTANCE == null) {
			synchronized (VmsGridPainter.class) {
				if (INSTANCE == null) {
					INSTANCE = new VmsGridPainter();
				}
			}
		}
		return INSTANCE;
	 ...
	 * @param viewPanel
	 */
	public void registerParentViewPanel(LiveViewPanel viewPanel) { this.view = viewPanel; }

	/**
	 * @return view
	 */
	public LiveViewPanel getRegisteredViewPanel() { return this.view; }
	 ...
	 */
	public void drawTileZoom(ImageTile tile, boolean fullscreenMode) { 
		view.removeAll();
		if (!fullscreenMode) {
			drawTiles(this.getCurrentGridLayout());
		} else { 
			currentSelectedTile = tile;
			view.setLayout(null);
			Dimension displayDimension = view.getSize();
			tile.setBounds(0, 0, displayDimension.width, displayDimension.height);
			tile.setDimension(displayDimension);
			view.add(tile);
			view.validate();
			view.repaint();
		}
	 ...
	 */
	public boolean drawTiles(VmsGridLayout layout) {
		if (this.isViewPanelRegistered()) {
			int tilesToDisplay = layout.getNoOfTiles();
			...
			boolean match = IntStream.of(1, 4, 9, 16, 25, 36).anyMatch(tileCount -> {
				return tilesToDisplay == tileCount;
			});
			
			if (match) {
				int currentX = 0;
				int currentY = 0;
				int divideRatio = (int) Math.sqrt(tilesToDisplay);
				...
				for (int index = 0; index < tilesToDisplay; index++) {
			...
			} else if (tilesToDisplay == 6) { /* for 5x1 layout */
			...
			} else if (tilesToDisplay == 8) { /* for 7x1 layout */
			...
			} else if (tilesToDisplay == 13) { /* for 12x1 layout */
				int oneFourthWidth = Math.round(currentDisplayWidth / 4);
				int oneFourthHeight = Math.round(currentDisplayHeight / 4);
				int twoFourthWidth = Math.round((currentDisplayWidth * 2) / 4);
				int twoFourthHeight = Math.round((currentDisplayHeight * 2) / 4);
	 ...
	 */
	public VmsGridLayout getCurrentGridLayout() {
