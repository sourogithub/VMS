...
public class SqliteAuditManagerImpl implements AuditManager {
	...
	private boolean createAuditDatabase() {
		...
		try {
			connection = DriverManager.getConnection(AuditSchema.AUDIT_DB_CONNECTION);
			statement = connection.createStatement();
			success = (statement.executeUpdate(createCommand) == EXECUTE_SUCCESS);
	...
	public VmsResponse generateAuditLog(AuditLog type, String description, Long time) {
		...
		final String insertCommand = "INSERT INTO " + AuditSchema.AUDIT_TABLE_NAME 
				+ " ( " + 
					AuditSchema.AUDIT_TYPE + " , "
				+ 	AuditSchema.AUDIT_DESC + " , " 
				+ 	AuditSchema.AUDIT_TIME 
				+ " ) " 
				+ " VALUES ( " 
					+ type.getID() 
				+ " , " + "'" + description + "'" 
		...
		try {
			...
			if (result != null) {
				...
				while (result.next()) {
					...
					AuditLog type = AuditLog.find(id);
					if (type != null) {
		...
		try {
			...
			if (result != null) {
				...
				while (result.next()) {
					...
					AuditLog audType = AuditLog.find(id);
					if (type != null) {
