...
typedef struct _StreamDataSpace
{
	...
	~_StreamDataSpace()
	{
		...
		avformat_close_input(&pFormatCtx); // replaced 231119
		...
		//delete vc;
		//delete buffer;
		//avcodec_close(inVideoStream->codec);

		//av_free(inVideoStream);

		//avcodec_close(outVideoStream->codec);
...
ErrorTypes TestInitialize()
{
	...
	createFolder("test_db");

	d->rc = sqlite3_open("./test_db/TEST_VMS_DB.db", &(d->db));
		...
		const CHAR8 *cameraURL, const CHAR8 *userId, const CHAR8 *pass, UINT32 camId)
{
	...
	std::cout << "AddStreamDEBUG: buffer allocated" << std::endl;

	//printf("abc = %d\n",camId);

	std::string url= sDataSpace->m_RtspURL;
	std::string urlDelimiter= "://";
	std::string protocol= url.substr(0,url.find(urlDelimiter));
	std::string domain= url.substr((url.find(urlDelimiter)+urlDelimiter.length()),url.length()-(protocol.length()+urlDelimiter.length()));
	/*std::string buf(sDataSpace->m_RtspURL);
	buf.append("?username=");
	buf.append(sDataSpace->m_UserId);
	buf.append("&password=");
	buf.append(sDataSpace->m_Password);*/
	std::string buf(protocol);
	buf.append("://");
	buf.append(sDataSpace->m_UserId);
	buf.append(":");
	buf.append(sDataSpace->m_Password);
...
void AddFrame(AVFrame *videoFrameTest, UINT8 *mapping) {
	...
	av_free(sDataSpace->pkt.data); // 231119
	av_free_packet(&sDataSpace->pkt); // 231119
}

bool isCameraConnected(struct sockaddr_in* serv_addr) {
	int sock = 0;
...
/*bool isCameraConnected(string url) {
	std::cout << "ISCONNECTED: " << url << std::endl;
	AVFormatContext* context;
	return (avformat_open_input(&context, url.c_str(), NULL,NULL) == 0);
}*/ // on 111119
...
ErrorTypes GetContent(UINT8 *dataSpace, UINT8 *contentBuff)
{
	...
	if(sDataSpace->count == 1){
		...
		//startTime= chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now().time_since_epoch());
		...
		/*if (sDataSpace->videoStream == -1)
			return DSS_ERROR; // Didn't find a video stream*/ // commented on 111119
		if (sDataSpace->videoStream == -1) {
			//if(!isCameraConnected(sDataSpace->url))
				//return DSS_IO_ERROR;
			//else
		...
		Initz(sDataSpace->pCodecCtx->width, sDataSpace->pCodecCtx->height, 10, 512, dataSpace, false);
		//pthread_mutex_unlock(&ml);
	...
	sDataSpace->packet.data = NULL;
	sDataSpace->packet.size = 0;

	std::cout << "DEBUG: Before reading" << std::endl;
	if (av_read_frame(sDataSpace->pFormatCtx, &(sDataSpace->packet)) >= 0) {
		if(sDataSpace->difference < TIME_DIFF)
		{
			std::cout << "DEBUG: After reading" << std::endl;
			...
			if (sDataSpace->packet.stream_index == sDataSpace->videoStream)
			{
				...
				/*memcpy(contentBuff + sizeof(int), &sDataSpace->packet.size, sizeof(int)); // image size
				memcpy(contentBuff + sizeof(int) + sizeof(int), &sDataSpace->packet.pts, sizeof(int64_t)); // time-stamp in millis
				// 32 bit time-stamp in nano
				memcpy(contentBuff + sizeof(int) + sizeof(int) + sizeof(int64_t) + sizeof(int), &sDataSpace->imageOffset, sizeof(int64_t)); // Start Point
				int mediaType= H264;
				memcpy(contentBuff + sizeof(int) + sizeof(int) + sizeof(int64_t) + sizeof(int) + sizeof(int64_t), &mediaType, sizeof(char)); // media type
				memcpy(contentBuff + sizeof(int) + sizeof(int) + sizeof(int64_t) + sizeof(int) + sizeof(int64_t) + sizeof(char), &sDataSpace->pFrame->pict_type, sizeof(char)); // frame type
				// 16 bit Reserve
				memcpy(contentBuff + sizeof(int) + sizeof(int) + sizeof(int64_t) + sizeof(int) + sizeof(int64_t) + sizeof(char) + sizeof(char) + sizeof(char) + sizeof(char), sDataSpace->packet.data, sDataSpace->packet.size); // image data
				sDataSpace->imageOffset += sizeof(int) + sizeof(int) + sizeof(int64_t) + sizeof(int) + sizeof(int64_t) + sizeof(char) + sizeof(char) + sizeof(char) + sizeof(char);*/
				...
				memcpy(contentBuff + sizeof(int) + sizeof(int) + sizeof(int64_t) + sizeof(int64_t), sDataSpace->packet.data, sDataSpace->packet.size); // commented on 27092019

				/* 27092019 */
				if(sDataSpace->count == 1){
					sDataSpace->startTime= chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now().time_since_epoch());
				}
				sDataSpace->count++;
				av_interleaved_write_frame(sDataSpace->vc->ofctx, &(sDataSpace->packet));
				/* ------- */
				#if 0
				FILE *fp;
				char name[50];
			...
			// Close the codec
			avcodec_close(sDataSpace->pCodecCtx); // commented on 25092019
			/*
			...
			*/
			...
			Finish(dataSpace, false);
			sDataSpace->count = 1;
		}
	}
	else
	{
		std::cout << "Failed to grab stream: Camera may get disconnected" << std::endl;
		return DSS_IO_ERROR;
	}
	return DSS_SUCCESS;
}
...
ErrorTypes TestGetContent(UINT8 *dataSpace, UINT8 *contentBuff, UINT8* encodedBuffer)
{
						...
						pthread_mutex_lock(&ml);
							...
							if(sDataSpace->count == 1){
								...
								Initz(sDataSpace->pCodecCtx->width, sDataSpace->pCodecCtx->height, 10, 512, dataSpace, true);
								//pthread_mutex_unlock(&ml);
							...
							std::cout << "DEBUG: Before reconstruction" << std::endl;
							INT32 bufferSize = *(int *)encodedBuffer;
							INT32 extra = sizeof(int) + sizeof(int64_t) + sizeof(int64_t);
...
{
	std::cout << "DEBUG: Removing Stream" << std::endl;
	StreamDataSpace *sDataSpace = (StreamDataSpace *)(*((UINT64 *)dataSpace + sizeof(char)));
	...
	std::cout << "DEBUG: Stream Removed" << std::endl;
	return DSS_SUCCESS;
}
...
void Initz(int width, int height, int fpsrate, int bitrate, UINT8 *mapping, bool isForTest) {

	pthread_mutex_lock(&ml);

	StreamDataSpace *sDataSpace = (StreamDataSpace *)(*((UINT64 *)mapping + sizeof(char)));
	if(sDataSpace->vc != NULL)
		delete sDataSpace->vc; // 231119
	...
	if(!isForTest) {
		...
		sprintf(sDataSpace->vc->ftemp, "tmpTest%d.h264", sDataSpace->m_camId);
	}
	...
	//std::cout << "codec_id:$$$$$$*********^^^^^^^^^:" << sDataSpace->vc->oformat->video_codec << std::endl;
	...
	sDataSpace->vc->videoStream->time_base = (AVRational){1, sDataSpace->vc->fps};
	sDataSpace->vc->videoStream->avg_frame_rate = (AVRational){1, sDataSpace->vc->fps};

	avcodec_parameters_to_context(sDataSpace->vc->cctx, sDataSpace->vc->videoStream->codecpar);
	sDataSpace->vc->cctx->time_base = sDataSpace->vc->videoStream->time_base;
	//sDataSpace->vc->cctx->time_base = (AVRational){ 1, sDataSpace->vc->fps};
	...
	char comBuff[50];
	memset(comBuff,'\0',50);
	sprintf(comBuff,"chmod 666 %s",sDataSpace->vc->ftemp);
	system(comBuff);
...
/*void AddFrame(AVFrame *videoFrameTest, UINT8 *mapping) {
	...
	//pthread_mutex_unlock(&ml);
}*/ // commented on 27092019

void Finish(UINT8 *mapping, bool isForTest) {
...
void Free(UINT8 *mapping) {
	...
	if (sDataSpace->vc->videoFrame) {
		...
		sDataSpace->vc->videoFrame= NULL;
	}

	printf("2\n");
	if(sDataSpace->vc->videoStream->codec) {
		printf("2A\n");
		avcodec_close(sDataSpace->vc->videoStream->codec);
		printf("2B\n");
	} // 231119

	/*printf("6\n");
	if(sDataSpace->vc->codec) {
		printf("6A\n");
		av_free(sDataSpace->vc->codec);
		printf("6B\n");
		sDataSpace->vc->codec= NULL;
	} // 231119*/

	if (sDataSpace->vc->cctx) {
		avcodec_free_context(&(sDataSpace->vc->cctx));
		sDataSpace->vc->cctx= NULL;
	}
	...
	if (sDataSpace->vc->ofctx) {
		...
		sDataSpace->vc->ofctx= NULL;
	}

	if (sDataSpace->vc->swsCtx) {
		sws_freeContext(sDataSpace->vc->swsCtx);
		sDataSpace->vc->swsCtx= NULL;
	}
	//printf("Success to free\n");

	/*printf("1\n");
	if(sDataSpace->vc->oformat) {
		printf("1A\n");
		av_free(&sDataSpace->vc->oformat);
		printf("1B\n");
		sDataSpace->vc->oformat= NULL;
	}*/
	...
	if(sDataSpace->vc->ifmt_ctx) {
		printf("4A\n");
		avformat_close_input(&sDataSpace->vc->ifmt_ctx); // replaced the avformat_free_context (experimental)
		printf("4B\n");
		sDataSpace->vc->ifmt_ctx = NULL;
	}

	printf("5\n");
	if(sDataSpace->vc->ofmt_ctx) {
		printf("5A\n");
		avformat_close_input(&sDataSpace->vc->ofmt_ctx); // replaced the avformat_free_context (experimental)
		printf("5B\n");
		sDataSpace->vc->ofmt_ctx = NULL;
	}

	/*printf("3\n");
	if(sDataSpace->vc->videoStream) {
		printf("3A\n");
		if(sDataSpace->vc->videoStream != NULL)
			av_freep(&sDataSpace->vc->videoStream);
		printf("3B\n");
		sDataSpace->vc->videoStream = NULL;
	} // 231119*/

}

static int writeLog(void *NotUsed, int argc, char **argv, char **azColName) {
	std::cout << "Video log written" << std::endl;
	return 0;
}

void Remux(UINT8 *mapping, bool isForTest) {
	...
	if(!isForTest) {
		...
		system("chmod 777 records");
		sprintf(sDataSpace->fname, "./records/camid_%d_%s.mp4", sDataSpace->m_camId, name_format);
	}
	else {
		createFolder("test_records");
		system("chmod 777 test_records");
		sprintf(sDataSpace->fname, "./records/test_camid_%d_%s.mp4", sDataSpace->m_camId, name_format);
	}
	...
	char comBuff[50];
	memset(comBuff,'\0',50);
	sprintf(comBuff,"chmod 666 %s",sDataSpace->fname);
	system(comBuff);
	...
	while (true) {
		...
		//		fclose(fp);
		//pthread_mutex_lock(&ml);
		av_init_packet(&(sDataSpace->videoPkt)); // 231119
		...
		//pthread_mutex_lock(&ml);
		sDataSpace->videoPkt.pts *= sDataSpace->vc->ifmt_ctx->streams[0]->codec->ticks_per_frame;
		sDataSpace->videoPkt.dts *= sDataSpace->vc->ifmt_ctx->streams[0]->codec->ticks_per_frame; // commented on 09102019
		if ((sDataSpace->err = av_interleaved_write_frame(sDataSpace->vc->ofmt_ctx, &(sDataSpace->videoPkt))) < 0) {
			...
			av_free(sDataSpace->videoPkt.data); // 231119
			av_free_packet(&sDataSpace->videoPkt); // 231119
			break;
		}
		//pthread_mutex_unlock(&ml);
		av_packet_unref(&(sDataSpace->videoPkt));
	...
	sDataSpace->endTime = chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now().time_since_epoch());

	std::string query= "INSERT INTO RECORD_LOG (CAM_ID,START_TIME,END_TIME,KEEP,URL) ";
	query.append("VALUES (");
	query.append(to_string(sDataSpace->m_camId));
	query.append(",");
	query.append(to_string(sDataSpace->startTime.count()));
	query.append(",");
	query.append(to_string(sDataSpace->endTime.count()));
	query.append(",");
	query.append("1");
	query.append(",'");
	query.append(sDataSpace->fname);
	query.append("');");
	std::cout << "Insert query: " << query.c_str() << std::endl;
	//d->sql= ;

	pthread_mutex_lock(&ml);
	d->rc = sqlite3_exec(d->db, query.c_str(), writeLog, 0, &(d->zErrMsg));
	pthread_mutex_unlock(&ml);

	free(sDataSpace->vc->ftemp);
	printf("\ninvid\n");
	//av_freep(&sDataSpace->inVideoStream->codec); // 231119
	//av_freep(&sDataSpace->inVideoStream); //231119
	//av_freep(&sDataSpace->outVideoStream->codec); // 231119
	//av_freep(&sDataSpace->outVideoStream); //231119
